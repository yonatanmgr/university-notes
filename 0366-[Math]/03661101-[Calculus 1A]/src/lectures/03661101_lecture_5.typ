...
#pagebreak()
=== דוגמה
$ liminff(sqrt(n)) (sqrt(n+1) -sqrt(n)) = limitn frac(sqrt(n)(sqrt(n+1) -sqrt(n))(sqrt(n+1) +sqrt(n)), sqrt(n+1)+ sqrt(n)) = \ 
  limitn frac((n+1)-n, sqrt(1+1/n)+1) = limitn 1/(sqrt(1+1/n)+1) = 1/2
$

== מונוטוניות הגבול
=== (למה) נתונה סדרה $(d_n)$ כאשר $d_n >= 0$ ו-$d_n->d$. אזי $d>=0$.
...
=== (הכללה של הלמה) נתונות שתי סדרות $(a_n)$ ו-$(b_n)$, ונניח $lim a_n =a$ ו-$lim b_n = b$ ו-$a_n <= b_n$. אזי $a<=b$.
...
== כלל הסנדוויץ׳
=== (משפט) כלל הסנדוויץ׳
נתונות שלוש סדרות $(x_n), (y_n), (z_n)$ כך ש-$(x_n)->L, (y_n)->L$. נניח גם ש-$ exists n_0 in NN : x_n <= z_n <= y_n, forall n < n_0 $ אזי $liminff(z_n)=L$.
...
=== (טענה) תהי קבוצה $B != emptyset$ חסומה מלמעלה. אז $exists (x_n) seq B$ כך ש-$lim x_n = sup B$.
...
==== הערה
אם קבוצה $B != emptyset$ חסומה מלמטה, אז $exists x_n in B$ לכל $n in NN$ כך ש-$liminff(x_n) = inf B$.
